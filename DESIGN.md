...
- Evidence reviewed: `flux-klein-studio(5).html`, `ui/public/index.html`, `ui/public/styles.css`, `ui/public/app.js`, `ui/lib/runtime.mjs`, `src/main.cpp`, and the accepted Visual Ralph artifacts under `.omx/artifacts/visual-ralph/flux-klein-studio/`.
...
- Goals: make FLUX.2 Klein generation and editing understandable as a visual image flow; keep common generation one action; expose connected edits without requiring CLI knowledge.
- Non-goals: a general ComfyUI replacement, arbitrary cyclic graphs, concurrent GPU execution, or controls unsupported by the native runtime.
...
- User jobs: generate an image from text, transform an existing generated image, chain several edits, inspect timings, and download results.
...
- New/changed components: Add Klein Node action, dynamic Klein node, inferred mode badge, removable connection, per-stage output selector.
...
- Offline/slow network: runtime status remains explicit; LoRA downloads and native jobs retain independent errors.
...
- Microcopy rules: describe the next action, name the affected stage, and never claim a downloaded LoRA is applied before native support exists.
...
- Performance constraints: serialize GPU jobs and fuse the first Generate to Edit pair through native `generate-edit` when compatible.
- Compatibility constraints: one image input per Edit node; no reference-strength control exists in the native CLI.
